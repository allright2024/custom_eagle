...
class CAbstractor(ConvProjector):
    ...
    def build_net(self):
        encoder_hidden_size = self.config.encoder_hidden_size # siglip(after fusion module) hidden_size
        hidden_size = self.config.hidden_size # c-abs hidden size 1024
        output_hidden_size = self.config.output_hidden_size # lanuage model(ex : synatra 1.3b) hidden_size
